...
## [ISS-032] 005DEVELOPMNET.md 首次构建运行指引对读者定位不准确
...
**发现日期**：2026-06-11
**解决日期**：2026-06-12
**解决方案**：重新定位目标读者为「想把项目跑起来看看能做什么的新手」，移除所有阶段标签、PLAN 引用、CI 守卫说明、开发者工作流内容，只保留从 clone 到看到产出的最短路径。

**现象**：
指引混入了大量面向「接手项目的开发者」的内容，对只想跑通看效果的新手造成噪音：
1. 目标读者描述包含「Phase 2.5 完工后的全部主干能力」等阶段标签，新手不理解阶段体系。
2. 正文、验证章节、进阶章节频繁引用 Phase 0.5 / 1A / 1B / 2 / 2.5 / 2.6，新手无法建立上下文。
3. 「11. 日常开发循环」和「12. 进阶」包含 CI 脚本说明、PLAN.md 行号引用、stage 命名规范等开发者专属内容，与「看懂项目」目标无关。
4. 验证步骤用「Phase 2 ★MVP 闭环」等表述，新手看不懂在验证什么。

**后果**：
- 目标读者（想快速了解项目能做什么的人）读到一半被阶段体系和 CI 规范劝退。
- 开发者专属内容与入门引导混在一起，主干流程被稀释。

**初步判断**：
已确认。指引存在两个完全不同的目标读者，需要按「新手快速入门」单一目标重写。

**关联**：
-> docs/005DEVELOPMNET.md
-> docs/004CHANGELOG.md [2026-06-12-1]
