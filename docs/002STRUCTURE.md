...
- `projects.py`        用户项目主线接口：上传素材、模板推荐、装配生成、查看项目数据、触发渲染、获取播放器属性、混入背景音乐。`GET /projects/{id}/recommendations` 从最新 `recommend_templates` 任务的事件流反查重组推荐结果，让 Editor 重进项目时恢复第二步卡片，不引入新的持久化字段。
...
- `phase1a_report.py`      视频素材识别阶段的中间报告，把切镜、画面字幕、贴纸、调色、镜头运动、画面构成等多个子能力的结果汇总到一棵树上。
...
- `captions.py`            画面字幕识别：先在每帧用 Canny + 横向形态学预扫高对比度长矩形作候选 ROI，再调 VLM 在 ROI 上判断是否字幕并给出位置、字号、颜色、描边、阴影、布局，跨场景去重合并；动画类型由 `understand/vision.py` 的 caption_function 子能力承担。
- `stickers.py`            VLM 网格采样识别贴纸的位置、语义类别，再用 Canny + 帧差细修边界框。
- `motion.py`              VLM 粗判每个分镜的镜头运动方向（推进 / 拉远 / 稳定 / 抖动 + 8 类平移），非稳定的再用光流估出关键帧曲线，每帧同时输出缩放比率与归一化平移量。
- `transitions.py`         VLM 看相邻 scene 边界前后三帧，分类转场类型（硬切 / 叠化 / 滑动 等）。
- `masks.py`               几何蒙版识别：CV 主路径只跑圆形检测（HoughCircles），矩形 / 分屏等其他形状统一走 VLM 兜底，VLM 提示中显式排除字幕、UI、水印等非蒙版元素。
- `b_roll.py`              画面构成识别：每个分镜取中间帧给 VLM 判断是「人物主导 / 全屏 B-roll / 画中画 / 侧栏」哪一类，含画中画 / 侧栏时附 ROI 框，供下游骨架阶段把对应槽位标记为「AI 生成画面」。
...
- `fill.py`                给缺口补内容：四种策略——文本模型生成补字幕、用占位风格做"包装型"片段、复用相邻片段尾帧做停留，或调第三方视频生成 API 补 AI 画面（仅当用户在编辑器勾选「允许 AI 补画面」时）；任何 AI 生成失败都自动降级为复用策略，不阻塞流水线。
...
- `aigc.py`                AIGC 生成层：B-roll 视频与贴纸图片的统一调度入口，按 (提示词, 风格, 时长) 的 sha256 做永久缓存避免重复支付 API 费，每次调用必发结构化事件，缺凭据 / 限额 / 内容审查 / 网络错误抛分类异常让上游降级。B-roll 的视频实际是「调文本生图 API 拿到静图后用 ffmpeg 把它循环成 N 秒 mp4」；视频的运动效果由模板里的镜头运动关键帧在渲染时附加，生成 API 只产出静态画面。
- `aigc_providers/`        AIGC 第三方适配层：每个文件对应一个图像生成厂商（当前只有 PPIO 网关的 OpenAI-compat `/v1/images/generations` 接口），子包按文件名映射到环境变量 `AIGC_BROLL_PROVIDER` 的值；只实现一个 `generate_image(提示词, 风格关键词)` 方法返回图片字节；完全不发事件、不反向依赖业务层，便于换厂商时只动一个文件。
